...
import { buildSourceMap } from './lib/shared-sourcing.mjs';
import {
  buildContentItems,
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--course-dir') {
      ...
      console.log('usage: node tools/retrieve-content.mjs --course-dir courses/<course-id> [--query-id <id>] [--source-id <id>] [--url <url-or-resource-page>] [--file <local-pdf-or-text>] [--limit 2] [--per-page 2] [--replace]');
      process.exit(0);
    } else {
...
async function main() {
  ...
  const queriesPayload = await readJson(await findCourseArtifactPath(coursePaths, 'CONTENT_RETRIEVAL_QUERIES'));
  ...
  if (queries.length === 0) {
    throw new Error('No content retrieval queries matched. Check --query-id or initialize content authoring first.');
  }
  ...
  console.log(`Retrieved ${collected.length} content candidate(s) for ${config.course_id}`);
